...
public class FancyLightRenderer<T extends FancyLightBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public FancyLightRenderer(BlockEntityRendererFactory.Context ctx) {
        this.fancylightModel = new FancyLightModel(FancyLightModel.getTexturedModelData().createModel());
    }
    ...
    public void render(T entity, float tickDelta, MatrixStack matrices,
        ...
        if (!(blockState.getBlock() instanceof FancyLightBlock)) {
            return;
        }

        Direction direction = blockState.get(FancyLightBlock.FACING);
        ...
        this.fancylightModel.render(matrices,
